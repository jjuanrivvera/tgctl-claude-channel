...
func TestKeyringAuthStatus(t *testing.T) {
	...
	st, err := keyringAuthStatus(Config{TgctlBin: valid})
	if err != nil {
	...
	if _, err := keyringAuthStatus(Config{TgctlBin: invalid}); err == nil {
		t.Fatal("invalid stored credentials must be rejected")
	}

	failing := fakeTgctl(t, `{}`, 1)
	if _, err := keyringAuthStatus(Config{TgctlBin: failing}); err == nil {
